...
### Order data
rawdata <- as.data.frame(fread("recommendation_data_20201221.csv"))
colnames(rawdata)
...
sum(orders$distorigintodest<1, na.rm=T) #배달거리 100m 미만 = 59491 obs(0.8%))
...
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   271.4  2061.5  2953.0  4181.4  4856.6 39245.4   59491
...
### data_2m_fullexist
#data_2m_fullexist <- orders.1 %>% filter(date>="2020-09-26" & date<="2020-11-30") %>%
#  filter(rider_id %in% rider_fullexist$rider_id)
#nrow(data_2m_fullexist); length(unique(data_2m_fullexist$rider_id)) #739,423건, 478명 라이더
